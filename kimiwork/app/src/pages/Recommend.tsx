...
export default function Recommend() {
  ...
  const { data: recommendations } = trpc.fund.recommendations.useQuery(
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-1">
                      {rec.tags?.slice(0, 3).map((tag: string) => (
                        <span key={tag} className="px-2 py-0.5 rounded text-[10px] font-medium bg-[#3B6CFF]/10 text-[#00F0FF] border border-[#3B6CFF]/20">
                          {tag}
                        </span>
                      ))}
                    </div>
                      ...
                      {rec.fundDetails?.map((fd: any, i: number) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              width: `${(fd.weight / totalWeight) * 100}%`,
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t border-white/[0.06]">
                      <div className="mb-4">
                        <h3 className="text-sm text-[#00F0FF] mb-2 flex items-center gap-1">
                        ...
                        {rec.fundDetails?.map((fd: any, i: number) => (
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <div className="text-white text-sm group-hover:text-[#00F0FF] transition-colors">
                                {fd.fund?.fundAbbr || fd.fund?.fundName}
                              </div>
                              <div className="text-white/25 text-xs data-number">
                                {fd.fund?.fundCode} · {fd.fund?.manager?.name}
                              </div>
                            </div>
