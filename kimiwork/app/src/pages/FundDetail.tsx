...
export default function FundDetail() {
  ...
  const { data: managerData } = trpc.fund.managerDetail.useQuery(
    { id: fund?.managerId || 0 },
    { enabled: !!fund?.managerId }
  );
  ...
  const radarData = useMemo(() => {
    ...
    const maxCalmar = 2;
    return [
      { metric: "收益率", fullMark: 100, value: Math.min(parseFloat(p.return1y || "0") + 50, 100), raw: p.return1y },
      { metric: "夏普比率", fullMark: 100, value: Math.min((parseFloat(p.sharpeRatio || "0") / maxSharpe) * 100, 100), raw: p.sharpeRatio },
      { metric: "卡玛比率", fullMark: 100, value: Math.min((parseFloat(p.calmarRatio || "0") / maxCalmar) * 100, 100), raw: p.calmarRatio },
      { metric: "胜率", fullMark: 100, value: parseFloat(p.winRate || "50"), raw: p.winRate },
      { metric: "抗回撤", fullMark: 100, value: Math.min(100 + parseFloat(p.maxDrawdown || "0") * 2, 100), raw: p.maxDrawdown },
      { metric: "Alpha", fullMark: 100, value: Math.min(parseFloat(p.alpha || "0") * 5 + 50, 100), raw: p.alpha },
    ];
  }, [fund]);

  if (isLoading) return <div className="min-h-screen pt-20 text-center text-white/30">加载中...</div>;
      ...
      <div className="max-w-7xl mx-auto px-6">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                {fund.isContinuousMarketing === 1 && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-[#3B6CFF]/15 text-[#00F0FF] border border-[#3B6CFF]/20">
                    持续营销
                  </span>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="liquid-glass p-6">
              <h2 className="text-lg font-medium text-white mb-4 flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-[#3B6CFF]" />
                业绩表现
              </h2>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={m.label} className="liquid-glass-sm p-3 text-center group hover:bg-white/[0.06] transition-all">
                    ...
                    <div className="data-number text-lg font-medium" style={{ color: m.color }}>
                      {m.value}{m.suffix}
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            ...
            <div className="liquid-glass p-6">
              <h2 className="text-lg font-medium text-white mb-4 flex items-center gap-2">
                <PieChart className="w-5 h-5 text-[#A3FF12]" />
                综合评分
              </h2>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
            ...
            {managerData && (
              <div className="liquid-glass p-6">
                <h2 className="text-lg font-medium text-white mb-4 flex items-center gap-2">
                  <User className="w-5 h-5 text-[#3B6CFF]" />
                  基金经理
                </h2>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <div className="text-white font-medium">{managerData.name}</div>
                    <div className="text-white/30 text-xs">{managerData.company} · {managerData.education}</div>
                    <div className="text-white/30 text-xs">从业{managerData.manageYears}年 · 管理{managerData.fundCount}只基金</div>
                  </div>
                </div>

                {managerData.styleDescription && (
                  <div className="mb-4">
                    <h3 className="text-xs text-[#00F0FF] mb-2 flex items-center gap-1">
                      <Zap className="w-3 h-3" />
                      AI 投资风格分析
                    </h3>
                    <p className="text-white/50 text-xs leading-relaxed">{managerData.styleDescription}</p>
                  </div>
                )}

                <div className="grid grid-cols-2 gap-2">
                  <div className="liquid-glass-sm p-2 text-center">
                    <div className="text-[#A3FF12] data-number text-sm font-medium">+{managerData.bestReturn}%</div>
                    <div className="text-white/25 text-[10px]">最佳年度</div>
                  </div>
                  <div className="liquid-glass-sm p-2 text-center">
                    <div className="text-[#FF3366] data-number text-sm font-medium">{managerData.worstReturn}%</div>
                ...
                <div className="mt-3">
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {managerData.investmentStyle?.split(",").map((s: string) => (
            ...
            <div className="liquid-glass p-6">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between">
                  <span className="text-white/30">管理费率</span>
                  <span className="data-number text-white/60">{(parseFloat(fund.feeManage || "0") * 100).toFixed(2)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/30">托管费率</span>
                  <span className="data-number text-white/60">{(parseFloat(fund.feeCustody || "0") * 100).toFixed(2)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/30">基金规模</span>
                  <span className="data-number text-white/60">{fund.totalScale}亿元</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/30">累计净值</span>
                  <span className="data-number text-white/60">{fund.accumNav}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/30">业绩基准</span>
                  <span className="text-white/60 text-right max-w-[200px]">{fund.benchmark}</span>
                </div>
              </div>
            </div>
          </div>
