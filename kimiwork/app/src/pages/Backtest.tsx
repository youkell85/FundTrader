...
export default function Backtest() {
  ...
  const { data: backtestList } = trpc.fund.backtests.useQuery();
  ...
  const [btParams, setBtParams] = useState<any>(null);
  const { data: btData, isFetching: btLoading } = trpc.fund.runBacktest.useQuery(
    btParams || { fundIds: [1], weights: [100], strategy: "fixed_amount", startDate: "2023-01-01", endDate: "2025-05-15", investAmount: 1000, investFrequency: "monthly" },
    { enabled: !!btParams }
  );

  useEffect(() => {
    if (btData && isRunActive) {
      setResult(btData);
  ...
  const selectedFundDetails = selectedFunds.map((id) =>
    fundList?.funds.find((f: any) => f.id === id)
  ).filter(Boolean);

  const displayResult = result || (isRunActive ? null : btData);
  const displayLoading = isRunActive || btLoading;
            ...
            <div className="liquid-glass p-5">
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  onChange={(e) => { if (e.target.value) { handleAddFund(parseInt(e.target.value)); e.target.value = ""; }}}
                  value=""
                >
                  <option value="">+ 添加基金</option>
                  {fundList?.funds.map((f: any) => (
                    <option key={f.id} value={f.id}>{f.fundAbbr || f.fundName} ({f.fundCode})</option>
                  ))}
                </select>
                      ...
                      <div key={f.id} className="flex items-center gap-2 liquid-glass-sm px-3 py-2">
                        ...
                        {selectedFunds.length > 1 && (
                          <input
                            ...
                            onChange={(e) => {
                              const newW = [...weights];
                              newW[i] = parseInt(e.target.value) || 0;
                              setWeights(newW);
                            }}
                  ...
                  {strategies.map((s) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-all ${
                        strategy === s.value
                          ? "bg-[#3B6CFF]/15 text-[#00F0FF] border border-[#3B6CFF]/20"
              ...
              <button
                onClick={handleRun}
                disabled={selectedFunds.length === 0 || displayLoading}
            ...
            {displayResult ? (
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: "总投入", value: `¥${parseFloat(displayResult.totalInvested || 0).toLocaleString()}`, color: "#3B6CFF" },
                    { label: "最终价值", value: `¥${parseFloat(displayResult.finalValue || 0).toLocaleString()}`, color: "#00F0FF" },
                    { label: "总收益率", value: `+${displayResult.totalReturn}%`, color: "#A3FF12" },
                    { label: "年化收益", value: `${displayResult.annualizedReturn}%`, color: "#FFB800" },
                    { label: "最大回撤", value: `${displayResult.maxDrawdown}%`, color: "#FF3366" },
                    { label: "夏普比率", value: displayResult.sharpeRatio, color: "#A3FF12" },
                  ].map((card) => (
                    <div key={card.label} className="liquid-glass-sm p-4 text-center group hover:bg-white/[0.06] transition-all">
                      ...
                      <div className="data-number text-xl font-medium" style={{ color: card.color }}>
                        {card.value}
                      </div>
                    </div>
                  ))}
                ...
                <div className="liquid-glass p-6">
                  ...
                  <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={displayResult.monthlyData || []}>
                        <defs>
                        ...
                        <Tooltip
                          contentStyle={{ background: "rgba(5, 8, 26, 0.95)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "8px", fontSize: "12px" }}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-white/30">超额收益</span>
                          <span className="data-number text-[#00F0FF]">+{displayResult.excessReturn}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/30">基准收益</span>
                          <span className="data-number text-white/50">{displayResult.benchmarkReturn}%</span>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-3">
                      ...
                      <p className="text-white/50 text-sm leading-relaxed">
                        {parseFloat(displayResult.sharpeRatio || 0) > 0.8
                          ? "该策略风险调整后收益表现优秀，夏普比率高于同类平均水平，建议作为核心配置策略。"
                          : parseFloat(displayResult.sharpeRatio || 0) > 0.5
                          ? "该策略具备合理的风险收益比，适合作为中长期定投方案。建议结合市场估值动态调整定投金额。"
                          : "该策略在当前回测区间表现一般，建议调整基金选择或定投参数以优化风险收益比。"}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="liquid-glass p-12 flex flex-col items-center justify-center text-center min-h-[500px]">
                ...
                <p className="text-white/30 text-sm max-w-md">
                  选择基金产品、设定定投参数，我们将基于历史数据为您计算策略的回测表现，包括收益率、最大回撤、夏普比率等核心指标
                </p>
              </div>
            )}
          </div>
