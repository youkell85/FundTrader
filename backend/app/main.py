...
from .api import fund, analysis, recommend, dca, professional, settings, allocation, storage, auth, admin_api, health, sales, marketplace, workspace
...
async def market_data_data_sources():

    from .allocation.data import market_data_service

    from .data.providers.fusion import get_fusion

    from .data.providers.tickflow_provider import TickflowQuotaPolicy
    ...
    return {
        ...
        "source": "tickflow_config",

        "market_data_service": market_data_service.get_status(),

        "providers": fusion_snapshot,

        "quotas": {

            "tickflow_paid": {

                "realtime_quotes": {"mode": "single", "limit": "120/min", "symbols": "100 symbols"},

                "historical_kline_1d_week_month": {"mode": "single", "limit": "120/min", "symbols": "1 标的/次"},
                "batch_klines": {"mode": "batch", "limit": "30/min", "symbols": "100 symbols"},

                "minute_bars": {"mode": "single", "limit": "120/min", "max_points": "5000"},

                "intraday_bars": {"mode": "single", "limit": "30/min", "symbols": "1 标的/次"},
                "market_depth": {"mode": "single", "limit": "60/min", "symbols": "1 标的/次"},
                "adjustment_factors": {"mode": "batch", "limit": "60/min", "symbols": "100 symbols"},

            },

            "tickflow_free": {

                "historical_klines": {"mode": "single", "limit": "120/min", "symbols": "1 标的/次"},
            },

            "quota_notes": [

                "1m/5m/15m/30m/60m: up to 5000 bars/interval",

                "Daily/weekly/monthly/quarterly/yearly: up to 365 calendar days",

                f"period.normalize sample={TickflowQuotaPolicy.normalize_period('1d')}",

            ],

            "supported_periods": ["1d", "1w", "1M", "1Q", "1Y", "1m", "5m", "15m", "30m", "60m"],
...
async def market_data_source_status():
    ...
    return market_data_service.get_market_data_health_snapshot()
...
async def market_data_stream(websocket: WebSocket, interval: int = 5):
    ...
    try:
        ...
        while True:
            ...
            payload = {
                ...
                "data": market_data_service.get_market_data_health_snapshot(),
