...
const PEER_RANKING_CATEGORIES = ["股票�?, "混合�?, "债券�?, "指数�?, "QDII", "FOF", "货币"];
...
function resolvePeerCategory(...values: unknown[]) {
  ...
  if (/�?.test(text)) return "债券�?;
  if (/指数|ETF|LOF|index|etf/i.test(text)) return "指数�?;
    ...
    .query(async ({ input }) => {
      try {
        ...
        return {
          ...
          source: "东方财富/AKShare 全市场同类基金排�?,
          updatedAt: new Date().toISOString(),
          rows: buildPeerPerformanceRows(target, peers),
        };
      } catch (err) {
        wrapError(err, "获取全市场同类收益排名失�?);
      }
    }),
      ...
      try {
        ...
        return {
          ...
          company: managerFund.manager.company !== "�?
            ...
            : managedFunds.find((fund: any) => fund.company && fund.company !== "�?)?.company || "基金公司待补�?,
  ...
  recommendations: publicQuery
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        const horizonConfig = horizon.includes("10�?)
          ? { key: "return10y", years: 10, label: "�?0�? }
          : horizon.includes("5�?)
            ? { key: "return5y", years: 5, label: "�?�? }
            : horizon.includes("3�?)
              ? { key: "return3y", years: 3, label: "�?�? }
              : horizon.includes("6个月")
                ? { key: "return6m", years: 0.5, label: "�?个月" }
                : { key: "return1y", years: 1, label: "�?�? };
        ...
        const themeMatches = (fund: any) => {
          ...
          if (focusTheme === "income") return /债|红利|股息|收益|现金|货币|短债|中短�?.test(text);
          ...
          if (focusTheme === "manufacturing") return /制造|高端|装备|军工|新能源|汽车|机器�?.test(text);
        ...
        const buildPortfolio = (variant: "defensive" | "balanced" | "growth", id: number) => {
          ...
          const allocations = slots.map((slot) => {
            ...
            return {
              ...
              reason: `${typeLabels[slot.type] || fund.category}仓位�?{horizonConfig.label}年化折算${round(stats.expected)}%，最大回�?{round(stats.drawdown)}%，估算夏�?{round(stats.sharpe, 2)}，在${names[variant]}目标下综合排序靠前。`,
              fund,
            };
          }).filter(Boolean);
          ...
          const sourceLabel = sourceMode === "watchlist" ? "自选基�? : sourceMode === "custom" ? "指定产品" : "鑫基�?;
          const constraints = [
            { label: "回撤约束", passed: expectedRisk <= maxDdLimit, value: `${round(expectedRisk)}% / ${maxDdLimit}%` },
            { label: "单品集中�?, passed: concentration <= 45, value: `${round(concentration, 0)}%` },
          ...
          const stressTests = [
            { label: "股债同时调�?, loss: round(-(expectedRisk * 0.65 + volatility * 0.08)), note: "权益和债券同时承压时的组合级估�? },
            { label: "权益急跌", loss: round(-(expectedRisk * (variant === "growth" ? 0.95 : 0.75))), note: "权益类仓位主导的短期冲击" },
            { label: "利率上行", loss: round(-(expectedRisk * 0.35 + (allocations.some((item: any) => item.fund?.fundType === "bond") ? 1.2 : 0))), note: "债券底仓净值回撤压�? },
          ];
          return {
            id,
            name: names[variant],
            description: `${sourceLabel} · ${horizon}视角，目�?{optimizationGoal === "risk" ? "控制波动" : optimizationGoal === "return" ? "提高收益弹�? : "收益风险平衡"}，约束最大回�?{maxDdLimit}%`,
            riskProfile,
            ...
            sourceLabel,
            rationale: `先按风险档位和投资周期确定大类权重，再在${sourceLabel}池内�?{horizonConfig.label}收益折算年化、最大回撤、估算波动、夏普、卡玛和数据完整度评分选基。组合层面用加权回撤、组合波动、CVaR95和集中度复核�?年�?0年优先使用对应长周期收益，缺失时使用最长可得净值年化。`,
            constraints,
            stressTests,
