...
- [analysis.py](file://v2/backend/app/api/analysis.py)
...
- [connection.ts](file://v2/frontend/api/queries/connection.ts)
- [analysis_service.py](file://v2/backend/app/services/analysis_service.py)
- [eastmoney_fetcher.py](file://v2/backend/app/data/eastmoney_fetcher.py)
</cite>
...
FundRouter["BFF路由层<br/>fund-router.ts"]
Home["首页页面<br/>Home.tsx"]
Client["API客户端<br/>fundtrader-client.ts"]
end
subgraph "后端(v2/backend)"
FastAPI["FastAPI 应用<br/>main.py"]
Config["配置与环境变量<br/>config.py"]
Cache["本地缓存管理<br/>cache_manager.py"]
AnalysisAPI["分析API<br/>analysis.py"]
...
- [fund-router.ts:1-623](file://v2/frontend/api/fund-router.ts#L1-L623)
- [Home.tsx:1-535](file://v2/frontend/src/pages/Home.tsx#L1-L535)
- [fundtrader-client.ts:1-172](file://v2/frontend/api/lib/fundtrader-client.ts#L1-L172)
...
- [analysis.py:1-78](file://v2/backend/app/api/analysis.py#L1-L78)
...
## 架构总览
前端通过 tRPC/HTTP 访问后端 API，后端根据请求参数调用服务层，服务层结合缓存与多数据源获取数据并返回。构建阶段进行代码分割与压缩，生产环境由 Nginx 提供静态资源与反向代理。新增的BFF层在前端和后端之间提供内存缓存和批量请求优化。

```mermaid
sequenceDiagram
participant Browser as "浏览器"
participant BFF as "BFF路由层(fund-router.ts)"
...
Browser->>BFF : 用户触发查询
BFF->>BFF : 检查内存缓存(30分钟TTL)
alt 命中缓存
BFF-->>Browser : 返回缓存值
else 未命中缓存
BFF->>TRPC : 发起批量请求
TRPC->>API : POST /analysis/batch
...
**更新** 新增BFF层的架构流程图，展示批量分析和内存缓存优化

图示来源
- [fund-router.ts:87-158](file://v2/frontend/api/fund-router.ts#L87-L158)
- [trpc.tsx:1-43](file://v2/frontend/src/providers/trpc.tsx#L1-L43)
- [api.ts:1-123](file://v2/frontend/src/lib/api.ts#L1-L123)
- [analysis.py:19-28](file://v2/backend/app/api/analysis.py#L19-L28)
- [main.py:1-41](file://v2/backend/app/main.py#L1-L41)
- [fund_service.py:1-193](file://v2/backend/app/services/fund_service.py#L1-L193)
- [cache_manager.py:1-54](file://v2/backend/app/data/cache_manager.py#L1-L54)
...
#### BFF路由层与批量请求优化
- 内存缓存策略：BFF层实现30分钟TTL的内存缓存，显著减少后端压力
- 批量分析接口：使用 `/analysis/batch` 接口一次性获取多个基金的分析数据，减少HTTP往返
- 首页加载优化：在 `fetchHomeFunds` 函数中，先获取所有基金列表，然后使用批量接口获取分析数据，最后合并结果
- 回退机制：批量请求失败时自动回退到单个请求，确保功能可用性

**更新** 新增BFF路由层的详细分析

章节来源
- [fund-router.ts:44-63](file://v2/frontend/api/fund-router.ts#L44-L63)
- [fund-router.ts:87-158](file://v2/frontend/api/fund-router.ts#L87-L158)
- [fundtrader-client.ts:57-63](file://v2/frontend/api/lib/fundtrader-client.ts#L57-L63)

#### 前端stale-time缓存策略
- 首页列表：`staleTime: 5 * 60 * 1000`（5分钟），适合频繁刷新的市场数据
- 市场概览：`staleTime: 5 * 60 * 1000`（5分钟），实时性要求较高的指标
- 筛选项：`staleTime: 30 * 60 * 1000`（30分钟），静态配置类数据
- 详情页：`staleTime: 5 * 60 * 1000`（5分钟），用户主动查看的详细信息

**更新** 新增前端stale-time缓存策略的详细说明

章节来源
- [Home.tsx:26-37](file://v2/frontend/src/pages/Home.tsx#L26-L37)
...
- 扩展缓存策略：新增 `CACHE_TTL_RANKING = 1800`（30分钟），用于排名类数据
...
- 批量分析接口：新增 `/analysis/batch` 端点，支持一次性获取多个基金的深度分析
...
#### 并行处理与超时优化
- 线程池并行处理：批量分析接口使用 `ThreadPoolExecutor(max_workers=8)` 实现并行处理，显著提升批量分析性能
- 超时配置：各数据源请求设置了合理的超时时间，避免长时间阻塞
- IO密集型优化：分析服务主要进行网络请求和文件缓存，适合并行处理模式

**更新** 新增并行处理能力和超时调整的详细说明

章节来源
- [analysis.py:20-36](file://v2/backend/app/api/analysis.py#L20-L36)
- [analysis_service.py:119-190](file://v2/backend/app/services/analysis_service.py#L119-L190)
- [eastmoney_fetcher.py:14-15](file://v2/backend/app/data/eastmoney_fetcher.py#L14-L15)
...
participant BFF as "BFF路由层"
participant Batch as "批量分析接口"
participant Detail as "单个分析接口"
Home->>BFF : 请求首页数据
BFF->>BFF : 检查内存缓存(30分钟)
alt 缓存命中
BFF-->>Home : 返回缓存数据
...
sequenceDiagram
participant BFF as "BFF路由层"
participant Pool as "线程池(8个worker)"
participant Service as "分析服务"
BFF->>Pool : 提交N个分析任务
loop 并行执行
Pool->>Service : analyze_fund(code)
Service-->>Pool : 返回分析结果
end
Pool-->>BFF : 汇总所有结果
BFF-->>BFF : 合并并缓存
...
图示来源
- [analysis.py:24-36](file://v2/backend/app/api/analysis.py#L24-L36)
- [analysis_service.py:119-190](file://v2/backend/app/services/analysis_service.py#L119-L190)
...
- 缓存策略
  ...
  - BFF层实现30分钟TTL的内存缓存，平衡性能与新鲜度

**更新** 新增BFF缓存策略和stale-time缓存策略的详细说明
...
- 内存管理
  ...
  - BFF层内存缓存30分钟TTL，避免重复计算
- 并发处理
  - 批量请求时并发限制与队列控制，避免数据源限流
  - 线程池并行处理提升批量分析性能，max_workers=8
- 缓存机制
  - 本地文件缓存与 Redis/Memcached 结合，提升命中率与共享性
  - 扩展缓存策略：排名类数据30分钟TTL，LLM评价12小时TTL

**更新** 新增批量分析接口、并行处理能力和扩展缓存策略的优化方法
...
- 后端
  ...
  - 批量请求成功率与响应时间监控
- 指标建议
  - 首屏时间、TTFB、PWA 指标、缓存命中率、错误率、响应时间分布、批量请求效率

**更新** 新增BFF层和批量请求的性能监控指标
...
- 前端
  ...
  - BFF缓存问题：检查内存缓存键生成与TTL设置
  - 批量请求失败：确认批量接口可用性和回退机制
- 后端
  - 缓存写入失败：检查缓存目录权限与磁盘空间
  - 数据源不可用：确认令牌与网络连通性，观察回源逻辑
  - 批量分析接口异常：检查代码列表和分析服务可用性
  - 并发处理异常：检查线程池配置和超时设置
- 部署
  - Nginx 静态资源 404：核对 base 与输出目录配置
  - 健康检查失败：检查端口与进程状态

**更新** 新增BFF层、批量请求和并发处理相关的故障排查指南
...
- [nginx_fund.conf](file://deploy/nginx_fund.conf)
- [fund-router.ts:114-134](file://v2/frontend/api/fund-router.ts#L114-L134)
- [analysis.py:24-36](file://v2/backend/app/api/analysis.py#L24-L36)

## 结论
通过合理的前端状态与网络层设计、后端缓存与回源策略、以及构建与部署层面的优化，FundTrader 可在保证功能完整性的同时显著提升性能与用户体验。新增的BFF层内存缓存和批量分析接口进一步优化了首页加载性能，30分钟TTL的缓存策略在保证数据新鲜度的同时大幅减少了后端压力。新增的并行处理能力（max_workers=8）和超时调整进一步提升了批量分析性能和系统的稳定性。建议持续监控关键指标，迭代优化缓存与并发策略，并逐步引入 CDN、PWA 与 SSR 等进阶手段。

**更新** 新增BFF层、批量分析接口、并行处理能力和超时调整的性能优化总结

## 附录
- 快速检查清单
  - 前端：manualChunks 是否合理、staleTime 是否匹配场景、是否开启 gzip、BFF缓存是否正常工作
  - 后端：缓存目录权限、TTL 是否合适、数据源限流与降级、批量分析接口可用性、线程池配置
  - 部署：Nginx 缓存与静态资源路径、健康检查与日志、批量请求监控
- 性能优化建议
  - 进一步优化批量请求的并发数量和超时设置
  - 考虑引入Redis缓存提升缓存命中率
  - 实施更细粒度的缓存失效策略
  - 监控BFF层内存使用情况，设置内存上限防止溢出
  - 根据实际负载调整线程池大小（max_workers）
  - 添加批量请求的超时控制和错误重试机制

**更新** 新增BFF层、批量请求、并行处理和超时调整的性能优化建议